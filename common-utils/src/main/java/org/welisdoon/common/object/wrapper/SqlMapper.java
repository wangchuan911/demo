...
//    }
    ...
    protected void build(GroupTable table, Map<String, Object> params) {
        BuildOption option = new BuildOption();
        option.params = params;
        table.setOption(option);
        table.build();
        sql = String.format("select %s %s", table.option.part.columns, table.option.part.tables);
    }
...
//            }
    ...
    public static class GroupTable extends AbstractTable<LinkColumn> {
        ...
        public void setOption(BuildOption option) {
            ...
            for (AbstractTable table : this.tables) {
                BuildOption option1 = new BuildOption();
