...
public class SqlShowBuilder implements SqlBuilder {
    ...
    protected String buildJoins(SqlContent content, String joinOpr, String condOpr, MetaLink metaLink) {
    ...
    @Override
    public String buildWhere(SqlContent content) {
        StringBuilder sql = Optional.ofNullable(buildWhereSql(content)).orElseGet(StringBuilder::new);
        while (sql.charAt(0) == ' ') {
            sql.delete(0, 1);
        }
    ...
    @Override
    public String buildJoins(SqlContent content) {
        ...
        stream.collect(Collectors.groupingBy(MetaLink::getType)).forEach((linkMetaType, list1) -> {
            ...
            buildJoins(content, sql, linkMetaType, list1);
        });
        return sql.toString();
    }

    protected void buildJoins(SqlContent content, StringBuilder sql, LinkMetaType type, List<MetaLink> list) {
        switch (type) {
            case SqlToJoinOfStrongRel:
                list.stream().forEach(metaLink -> {
                    sql.append(buildJoins(content, "join", "on", metaLink));
                });
                break;
            case SqlToJoinOfWeakRel:
                list.stream().forEach(metaLink -> {
                    sql.append(buildJoins(content, "left join", "on", metaLink));
                });
                break;
        }
    }

    public StringBuilder buildWhereSql(SqlContent content) {
        StringBuilder sql = Optional.ofNullable(content.getParent()).map(this::buildWhereSql).orElseGet(StringBuilder::new);
        List<LinkMetaType> linkMetaTypes = content.getLinkMetaTypes();
        Stream<MetaLink> stream = content.getLinks().stream();
        if (content.getParent() == null) {
            MetaLink mainTable = content.getLinks().stream().findFirst().orElseThrow(() -> new IllegalStateException("缺少主表"));
        ...
        stream.collect(Collectors.groupingBy(MetaLink::getType)).forEach((linkMetaType, list1) -> {
            ...
            buildWhereSql(content, sql, linkMetaType, list1);
        });
    ...
    protected void buildWhereSql(SqlContent sqlContent, StringBuilder sql, LinkMetaType type, List<MetaLink> list) {
        switch (type) {
    ...
    @Override
    public String buildColumns(SqlContent content) {
        return "*";
    }
